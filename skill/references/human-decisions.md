...
必须确认：目标、范围、非目标、事实源、治理单元、项目定位、当前交付目标、项目预期发行状态、产品语义、R2/R3 case、主观体验、发布、安全、权限、数据破坏和不可逆操作。
...
边界仍未知时，一个阻断问题只能改变一个变量，禁止把“确认摘要”与尚未回答的平台、技术栈或范围选择混在一起。已由用户或权威事实源明确的轴不得重复询问；所有边界事实稳定后，用**一次综合确认**同时绑定需求摘要、`KEY_OBJECTIVES.md` 与规范化 positioning。综合确认只能复述已解决事实和剩余 `UNKNOWN`，不得夹带新的架构或产品选择。

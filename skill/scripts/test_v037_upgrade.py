...
def test_plan_is_read_only_and_apply_invalidates(base: Path, package: Path) -> None:
    ...
    actual = [
        ...
        for path in sorted(value for value in (archive / "control-plane").rglob("*") if value.is_file())
    ]
...
def test_dirty_wrong_hash_untracked_and_plan_drift(base: Path, package: Path) -> None:
    ...
    project, source, target = make_project(base, package, "negative")
    spec = add_upgrade_spec(project, source, target)
    _, planned = command(package, project, "--plan", "--spec", str(spec), expect=2)
    plan_hash = planned["data"]["planHash"]
...
def test_staging_failure_preserves_live_control(base: Path, package: Path) -> None:
    project, source, target = make_project(base, package, "rollback")
    spec = add_upgrade_spec(project, source, target)
    ...
    try:
        ...
        with mock.patch.object(
            ...
            try:
                ...
                assert exc.check_id == "HC-UPGRADE-STAGING", exc.check_id
            else:
                raise AssertionError("expected injected staging failure")
        assert control_snapshot(project) == before
        assert not (project / f".vibe-control.upgrade-{planned['planHash'][:12]}.tmp").exists()
        assert not (project / f".vibe-control.upgrade-{planned['planHash'][:12]}.backup").exists()
    finally:

...
自动模式普通阶段不等待批准；负责上下文是控制面、里程碑提交和已授权非强制推送的唯一写入者。候选闭合、人工 checkpoint/owner decision、边界变化、R3/不可逆操作、硬失败、推送冲突或用户中断时必须停止，并运行 `python .vibe-control/runtime/0.3.7/control.py dashboard --project .` 生成外部缓存的只读、非权威复核快照。
<!-- vibe-control:end -->
